...
import os
import re
from getnewdata1 import getnew
...
class Data(object):
    def __init__(self, symbol,barinterval,datalist, keyList=keyList, initcash=StartMoney, everyhand=EveryHand):
        self.Symbol=symbol
...
class Process(object):
    ...
    def process(self,TBdir,SSdir):
        ...
        aftertime=self.strategy.data.Date[0]*10000+self.strategy.data.Time[0]
        print("aftertime",aftertime)
        os.system("pause")
        while True:
            # print("aftertime:",aftertime)
            newdata_list = getnew(SSdir, aftertime)
            if newdata_list is None: continue
            print(newdata_list)
            lastdata = newdata_list[-1]
            aftertime = lastdata[0] * 10000 + lastdata[1]
            pretime = (self.strategy.data.Date[0], self.strategy.data.Time[0])
            for newdata in newdata_list:
                ...
                if  (nowtime[0] == pretime[0] and nowtime[1] > pretime[1]) or (nowtime[0] > pretime[0]):
                    print('add data')
