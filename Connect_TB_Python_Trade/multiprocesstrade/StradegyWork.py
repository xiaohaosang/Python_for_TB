...
def init_Tradeinfos(opera_mysql,Tradeinfo_list,ConstantData):
    for tradeinfo in Tradeinfo_list:
        ...
        #print(tradeinfo['process'].symbol)
...
def mergeConstantdata(symbol,historydata,ConstantData):  #结合数据库和时时数据
...
'''
    ...
    #接受的行情数据进行字典处理，maxlen为保留的最大bar数量
    ...
    for symbol,data in deque:
        if symbol in ConstantData.keys():
            ...
            if maxlen is not None and len(ConstantData[symbol]) >maxlen:
                ...
                lastdata = ConstantData[symbol][-1]
                #同一根bar替换，下一根bar增加，因为是多进程，共享内存不能直接赋值处理，需要中间变量，即id已变，故需要maxlen，保证效率
                if data[0]==lastdata[0] and data[1]==lastdata[1]:
    ...
    if symbol in ConstantData.keys():
        ...
        while i<=len(data_list):
            ...
            if (newdata[1] > predata[1]) or (newdata[0] > predata[0]):
                ...
                if newdata!=predata:  #需要这一句，TradeOperaWork传出的是最新对象，但ConstantData[symbol]保存的是序列，序列里有更新的和之前的对象，所以需要判断
                    newdata_list.append(newdata)
                else:
                    break
            i=i+1
...
class MultiProcess(Process):
    def __init__(self,symbol, data,stradegy_tuple):
        ...
        self.symbol_stradegy=symbol+'_'+self.strategy.__class__.__name__
        #print("self.strategy_param:",self.strategy_param  )


    def process(self,TBdir,ConstantData,mulqueue):
        ...
        while self.strategy.data.currentbar< self.strategy.data.barcount:
            if  self.strategy.data.currentbar == self.strategy.data.barcount-1:
                ...
                break
            self.strategy(**self.strategy_param)
            mulqueue.put((self.symbol_stradegy,(self.strategy.data.Date,self.strategy.data.Time),self.strategy.data.oneBarInfo))
        ...
        while True:
            ...
            #print("predata:",predata)
            newdata_list = multigetnew(self.symbol,ConstantData,predata)  #得到最新数据
            ...
            for newdata in newdata_list:
                if newdata[0] == predata[0] and newdata[1] == predata[1]:
                    ...
                    self.strategy(**self.strategy_param)
                    #print(self.strategy.data.oneBarInfo)
                    mulqueue.put((self.symbol_stradegy, (self.strategy.data.Date, self.strategy.data.Time),self.strategy.data.oneBarInfo))
                    self.strategy.data.oneBarInfo = []
...
def make_symbol_stradegy_list():
    symbol_stradegy_list=[]
    for key in Tradeinfo_list:
        symbol_stradegy=Tradeinfo_list[symbol]+'_'+stradegy_tuple[0]
        symbol_stradegy_list.append(symbol_stradegy)
    return symbol_stradegy_list


def send_mulqueue(mulqueue,symbol_stradegy_list):
    s = socket.socket()
    host = 'DESKTOP-O73ROK7'
    port = 33333
    s.connect((host, port))
    ...
    #传送symbol_stradegy_list，TradeOperaWork创建文件夹
    print("send symbol_stradegy_list:",symbol_stradegy_list)
    pickledata = pickle.dumps(symbol_stradegy_list)
...
class Strategy1(BaseStrategy):
    ...
    def script(self):
        ...
        self.data.oneBarInfo.append(("commentary",(self.data.Symbol,self.data.BarInterval,self.data.currentbar,self.data.Time[0],self.data.Open[0],self.data.High[0],self.data.Low[0],self.data.Close[0],self.data.Vol[0],self.data.OpenInt[0])))
        '''
...
if __name__ == '__main__':
    ...
    #开启传输tradeinfo的线程
    mulqueue = multiprocessing.Queue()
    symbol_stradegy_list=make_symbol_stradegy_list()
    threading.Thread(target=send_mulqueue, args=(mulqueue,symbol_stradegy_list,)).start()
    time.sleep(2)

    # 处理初始化信息，包括申请数据库历史数据，合并内存中的时时数据(接收行情数据的线程先开)，生成MultiProcess的对象
    opera_mysql = OperaMysql('root', '123456', table='five_min_data')
    init_Tradeinfos(opera_mysql,Tradeinfo_list,ConstantData)

    #开启策略多进程
    process_list = [multiprocessing.Process(target=tradeinfo['process'].process, args=(tradeinfo['tbdir'],ConstantData,mulqueue)) for tradeinfo in Tradeinfo_list]
